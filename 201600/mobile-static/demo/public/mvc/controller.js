...
//        }
    ...
    historyShowView: function(direct){
        if(location.hash){
            ...
            if(newView){
                ...
                $(oldView).removeClass('current');
                $('#wrapper').loadding();
                this.view[url] = $('<div class="current page" id="' + url + '"><div class="main"></div></div>')[0];
                $('#container').append(self.view[url]);
                this.loadView(function(){
                    self._direction = 'right';
                    self.switchView(oldView,this.view[url]);
                });
            }
            this.ja();
        }
    ...
    showView: function(){
        ...
        if(newView){
            ...
            if(oldView == newView){
                ...
                $('#wrapper').loadding();
                this.loadView(function(){
                    self.switchView(oldView, newView);
                });
            }
        }else{
    ...
    createElement: function(){
        if(this._emptyElement == null){
            ...
            $(this._emptyElement).addClass('page');
        }
    ...
    switchView: function(container, new_container){
        ...
        var transitionEnd = function(e){
            $(this).attr('style', '');
            this.removeEventListener('webkitAnimationEnd', transitionEnd, false);
        ...
        $container.show();
        container.addEventListener('webkitAnimationEnd', transitionEnd, false);
        $container.attr("page-id", "old-container");
        $new_container.attr({"page-id": "container"});
        new_container.addEventListener("webkitAnimationEnd", transitionEnd, false);
        if(this._direction=='left'){
                $new_container.removeClass('out').addClass('slide').addClass('in');
                $container.removeClass('in').addClass('slide').addClass('out');
            ...
            // },0)
                ...
                $new_container.removeClass('out').addClass('reverse').addClass('in');
                $container.removeClass('in').addClass('reverse').addClass('out');
            // $new_container.attr("style", "display:block;-webkit-transform: translate3d(-100%, 0, 0)");
            //     setTimeout(function(){
            //     $container.attr("style", "display:block;-webkit-transition: all 0.3s ease-in-out;-webkit-transform: translate3d(100%,0,0)");
            //     $new_container.attr("style", "display:block;-webkit-transition: all 0.3s ease-in-out; -webkit-transform: translate3d(0,0,0)");
