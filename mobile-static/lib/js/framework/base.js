...
var Jumei = (function(window, scale) {
    ...
    J.mix(J, {
         ...
         * @desc 实例化组件
         */
        newModule: function(ns) {
            var classWidget = J.getModule(ns);
            return new classWidget();
        },
        /**
         * @desc 获取组件单例
         */
        getInstance: function(ns, args) {
            if (cache[ns]) {
                return cache[ns];
            } else {
                var classWidget = J.getModule(ns);
                cache[ns] = new classWidget(args);
                return cache[ns];
            }
        }
    });
    ...
    try {
        if (deviceAgent.match(/(iphone|ipod|android|windows\s*phone|symbianos)/)) {
            if (window.localStorage && window.localStorage.getItem('jumei_scale_320')) {
                scale = window.localStorage.getItem('jumei_scale_320');
            } else {
                scale = parseFloat(windowWidth / 320);
                if ($(window).height() > $(window).width()) {
                    window.localStorage && window.localStorage.setItem('jumei_scale_320', scale);
                } else {
