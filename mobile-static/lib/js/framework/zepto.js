...
var Zepto = (function() {
  ...
  $.fn = {
    ...
    scrollTop: function(){
        if (!this.length) return;
            ...
            return scrollTop/(Jumei && Jumei.getScale() || 1);
    },
    scrollTo: function(val){
      if(isNumber(val)){
        val = val * (Jumei && Jumei.getScale() || 1);
  ...
  ;['width', 'height'].forEach(function(dimension){
    $.fn[dimension] = function(value){
      ...
      if (value === undefined){ 
        var result = isWindow(el) ? el['inner' + Dimension]/(Jumei && Jumei.getScale() || 1) : isDocument(el) ? el.documentElement['offset' + Dimension]/(Jumei && Jumei.getScale() || 1) :
  ...
  function add(element, events, fn, selector, getDelegate, capture){
    ...
    eachEvent(events, fn, function(event, fn){
      ...
      handler.proxy = function (e) {
          var scale = (Jumei && Jumei.getScale() || 1);
