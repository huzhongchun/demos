...
var LeyouObject = function(scale){
    this.scale = scale;
}
LeyouObject.prototype.getScale = function(){
    return this.scale;
}
var Leyou = new LeyouObject(
    (function() {
    var scale = 1,
...
var Zepto = (function() {
  var undefined, key, $, classList, emptyArray = [], slice = emptyArray.slice, filter = emptyArray.filter,
    ...
    getComputedStyle = function(dom,cssName){
        if(document.defaultView.getComputedStyle){
            return document.defaultView.getComputedStyle
        }
        else{
            // for ie
            return cssName ? dom.currentStyle[cssName] : dom.currentStyle
        }
    },
  ...
  $.fn = {
    ...
    scrollTop: function(){
        if (!this.length) return;
            ...
            return scrollTop/(Leyou && Leyou.getScale() || 1);
    },
    scrollTo: function(val){
      if(isNumber(val)){
        val = val * (Leyou && Leyou.getScale() || 1);
  ...
  ;['width', 'height'].forEach(function(dimension){
    $.fn[dimension] = function(value){
      ...
      if (value === undefined){ 
        var result = isWindow(el) ? el['inner' + Dimension]/(Leyou && Leyou.getScale() || 1) : isDocument(el) ? el.documentElement['offset' + Dimension]/(Leyou && Leyou.getScale() || 1) :
  ...
  function add(element, events, fn, selector, getDelegate, capture){
    ...
    eachEvent(events, fn, function(event, fn){
      ...
      handler.proxy = function (e) {
          var scale = (Leyou && Leyou.getScale() || 1);
